document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add minimal mouse movement animation for profile image
    const profileContainer = document.querySelector('.profile');
    const profileImage = document.querySelector('.profile-image');
    
    if (profileContainer && profileImage) {
        // Create a notification to show animation is available
        const notification = document.createElement('div');
        notification.textContent = '✨ Move your mouse over my photo!';
        ...
        // Make initial animation more noticeable
        setTimeout(() => {
        ...
        profileContainer.addEventListener('mousemove', (e) => {
            ...
            // Calculate the offset (increased to 20px movement for visibility)
            const xOffset = ((x / rect.width) - 0.5) * 20;
            const yOffset = ((y / rect.height) - 0.5) * 20;
            
            // Apply transform to the profile image with more pronounced rotation
            profileImage.style.transform = `translate(${xOffset}px, ${yOffset}px) rotate(${xOffset * 0.5}deg)`;
            profileImage.style.transition = 'transform 0.15s ease-out';
        });
        
        // Reset position when mouse leaves
        profileContainer.addEventListener('mouseleave', () => {
            profileImage.style.transform = 'translate(0, 0) rotate(0deg)';
            profileImage.style.transition = 'transform 0.5s ease-out';
        });
    } else {
        console.log('Profile animation: Elements not found', 
                   {container: profileContainer, image: profileImage});
    }
